...
import { fetchMaterials, rateMaterialDB, fetchSettings, type Material, type AppSettings } from "@/lib/supabase-materials";
import { incrementDownloads, isBookmarked, toggleBookmark, MATERIAL_TYPES } from "@/lib/store";
...
const RankersLibrary = () => {
  ...
  const handleRate = async (m: Material, rating: number) => {
    await rateMaterialDB(m.id, m.ratings, rating);
    await loadData();
  };
  ...
  if (loading) {
    return <div className="card-elevated p-16 text-center text-muted-foreground">Loading materials...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Search & Filter */}
      <motion.div className="flex flex-col sm:flex-row gap-3" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
        ...
        {["All", ...MATERIAL_TYPES].map((t) => (
          <button
            ...
            className={`text-xs px-3 py-1.5 rounded-full font-medium transition-all duration-200 ${
              selectedType === t ? "bg-primary text-primary-foreground shadow-sm" : "bg-muted text-muted-foreground hover:bg-muted/80"
            }`}
          >
            {t}
          </button>
      ...
      {/* Materials */}
      {filtered.length === 0 ? (
        <motion.div className="card-elevated p-16 text-center" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
              ...
              <div className="flex items-center gap-3 shrink-0">
                {settings.ratingEnabled && m.ratingEnabled && (
                  <div className="flex items-center gap-0.5">
                    {[1, 2, 3, 4, 5].map((r) => (
                      <button key={r} onClick={() => handleRate(m, r)} className="text-primary/30 hover:text-primary transition-colors hover:scale-110">
